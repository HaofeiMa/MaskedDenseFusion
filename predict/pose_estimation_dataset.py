...
use_cuda = True
dataset = "coco"
ckpt_path = "./check_points/maskrcnn_coco-20.pth"
...
if __name__ == '__main__':
    ...
    pt = []
    pt.append(ply_vtx('{0}/models/obj_{1}.ply'.format(root, '%02d' % 1)))
    pt.append(ply_vtx('{0}/models/obj_{1}.ply'.format(root, '%02d' % 2)))
    pt.append(ply_vtx('{0}/models/obj_{1}.ply'.format(root, '%02d' % 3)))
    pt.append(ply_vtx('{0}/models/obj_{1}.ply'.format(root, '%02d' % 4)))



for item in range(1700):
    # 图像读取
    image = Image.open('{0}/data/{1}/rgb/{2}.png'.format(root, '%02d' % obj2rec, '%04d' % item))  # 读取图片
    depth = np.array(Image.open('{0}/data/{1}/depth/{2}.png'.format(root, '%02d' % obj2rec, '%04d' % item)))    # 读取深度
    # label = np.array(Image.open('{0}/data/{1}/label/{2}_label.png'.format(root, '%02d' % obj, '%04d' % item)))    # 读取标签

    # Mask RCNN 获取掩码
    img = Image.open('{0}/data/{1}/rgb/{2}.png'.format(root, '%02d' % obj2rec, '%04d' % item))  # 读取图片
    img = img.convert("RGB")
    img = transforms.ToTensor()(img)
    img = img.to(device)
    with torch.no_grad():
        result = model(img)   # 获取预测结果
    ...
    for obj in objs:    # 对结果中所有物体进行估计
        ...
        image = cv2.imread('{0}/data/{1}/rgb/{2}.png'.format(root, '%02d' % obj2rec, '%04d' % item))  # 读取图片
        point_size = 1
