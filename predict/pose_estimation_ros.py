...
def find_non_overlapping_boxes(boxes, scores):
    """
    从boxes列表中找到不相交的box，返回索引列表
    当box相交时，保留score值较大的那个
    
    Args:
    boxes: box(x1, y1, x2, y2)的列表
    scores
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if depth_image is None or rgb_image is None:
            continue
        image_cv2 = cv2.cvtColor(cv2.cvtColor(rgb_image, cv2.COLOR_BGR2RGB), cv2.COLOR_BGR2RGB)
        depth_cv2 = depth_image
        image_maskrcnn = Image.fromarray(cv2.cvtColor(image_cv2,cv2.COLOR_BGR2RGB))
        image_maskrcnn = image_maskrcnn.convert("RGB")
        image_maskrcnn = transforms.ToTensor()(image_maskrcnn)
        image_maskrcnn = image_maskrcnn.to(device)
        # target = {k: v.to(device) for k, v in target.items()}

        # 利用MaskRCNN网络获得结果
        with torch.no_grad():
            result = model(image_maskrcnn)
        
        # 判断结果
        if isinstance(image_maskrcnn, torch.Tensor) and image_maskrcnn.dim() == 3:
            images_maskrcnn = [image_maskrcnn]
        if isinstance(result, dict):
            targets = result    # [result]

        # 进行标注
        class_names = classes
        for i in range(len(images_maskrcnn)):
        ...
        # 进行位姿估计
        for obj_index, obj_num in enumerate(labels):
            ...
            # 获取深度点云数据
            depth_densefusion = np.array(Image.fromarray(depth_cv2))
            image_densefusion = Image.fromarray(image_cv2)
            points, choose, img, idx = rgbd2cloud(image_densefusion, depth_densefusion, label, obj)
            ...
            for point in points_list:
                cv2.circle(image_cv2, point, point_size, point_color, point_thickness)

            cv2.imshow("RGB Image", image_cv2)
        cv2.waitKey(1)
