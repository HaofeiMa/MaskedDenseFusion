...
# train command
# python train.py --use-cuda --iters 100 --dataset coco --data-dir "/media/huffie/One Touch/Dataset/coco"

    
def main(args):
    device = torch.device("cuda" if torch.cuda.is_available() and args.use_cuda else "cpu") # 如果cuda可用就用cuda，否则用CPU
    if device.type == "cuda": 
        pmr.get_gpu_prop(show=True) # 获取CUDA设备的数量、每个设备的名称、计算能力、总内存大小、内存时钟、内存带宽等属性
    ...
    d_test = pmr.datasets(args.dataset, args.data_dir, "val2017", train=True) # 导入测试数据集 set train=True for eval
        
    args.warmup_iters = max(1000, len(d_train)) # 设置预热迭代次数
    
    # -------------------------------------------------------------------------- #

    print(args)
    num_classes = max(d_train.dataset.classes) + 1 # 包含背景分类
    model = pmr.maskrcnn_resnet50(True, num_classes).to(device)
    
    params = [p for p in model.parameters() if p.requires_grad] # 读取模型参数
    optimizer = torch.optim.SGD(
        params, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay) # 使用随机梯度下降SGD来优化模型参数
    lr_lambda = lambda x: 0.1 ** bisect.bisect(args.lr_steps, x)    # 学习率衰减函数，当epoch数增加一倍时，学习率降低10倍
    ...
    ckpts = glob.glob(prefix + "-*" + ext)  # 寻找与给定模型路径匹配的所有权重文件
    ckpts.sort(key=lambda x: int(re.search(r"-(\d+){}".format(ext), os.path.split(x)[1]).group(1))) # 排序找到最后一个权重文件
    if ckpts:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        iter_train = pmr.train_one_epoch(model, optimizer, d_train, device, epoch, args)    # 训练一次
        A = time.time() - A
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", default="/home/huffie/Documents/ImageProcessing/PyTorch-Simple-MaskRCNN/dateset")
